...
我们都知道，当多张连续图像以一定的速率（比如每秒 24 张）播放，即可形成视频。但视频传输却并不是单纯地发送连续图像这么简单。假设一张图像分辨率为 1280x720，且每个 RGB 像素点大小为 24bit，则单张图像大小约为 21.1Mb；如果每秒需要传输 24 张，则大约需要占用约 506.4Mb 的带宽。显然这样的带宽占用是不现实的。
